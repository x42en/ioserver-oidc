...
# Optional — expected `aud` claim (RFC 8707 resource mode)
AUTH_SERVICE_AUDIENCE=https://api.example.com
```
...
| `AUTH_SERVICE_AUDIENCE` | No | Expected `aud` claim for RFC 8707 resource mode (default: `AUTH_SERVICE_APP_SLUG`) |
