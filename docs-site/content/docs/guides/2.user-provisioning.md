...
appHandle.users.findOrCreate(sub, { email, name })
       ...
       │
       ▼
isDisabled === true → 403 ERR_ACCOUNT_DISABLED
...
To disable a user, set `isDisabled: true` in your local DB. On the next request or connection attempt the middleware will reject with `403` / `ERR_ACCOUNT_DISABLED` **even if the JWT is still valid**. This provides instant revocation without waiting for the token to expire.
...
`OidcSocketMiddleware` also calls `appHandle.session.registerSocket(socket, namespace, email, name)` if available (and `unregisterSocket(socketId)` on disconnect). The `socket` already carries the injected `userId`, `sub`, and `roles`. Implement a `"session"` manager exposing those methods to track active WebSocket connections:
...
class SessionManager extends BaseManager {
  private sockets = new Map<
    string,
    { userId: string; sub: string; namespace: string }
  >();

  registerSocket(
    socket: Socket & { userId: string; sub: string },
    namespace: string,
    _email: string | null,
    _name: string | null,
  ) {
