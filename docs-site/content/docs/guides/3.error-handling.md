...
| `ERR_AUTH_TOKEN_INVALID` | 401 | JWT signature or claims verification failed (expired, wrong issuer, wrong audience, wrong/missing `azp`, disallowed algorithm, bad signature) |
| `ERR_ACCOUNT_DISABLED` | 403 | Token is valid but the local user account is disabled |
...
**Signature verification** — Every token is verified against the remote JWKS using `jose.jwtVerify`, restricted to an asymmetric algorithm allow-list (`RS*`, `PS*`, `ES*`, `EdDSA`). Tokens signed with a symmetric algorithm (`HS*`) or `none` are rejected to prevent algorithm-confusion attacks. Override the list via `OidcConfig.algorithms`.

**Claim validation** — `iss` (issuer) and `aud` (audience) are always validated. `aud` matches `OidcConfig.appSlug` by default, or `OidcConfig.audience` (a resource URL) in RFC 8707 mode. The `azp` (authorized party) claim is bound to `OidcConfig.appSlug` — mandatory in resource-audience mode — to prevent token substitution between different applications sharing the same auth-service.
