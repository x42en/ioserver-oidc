...
- Signature algorithm outside the allow-list (`config.algorithms`, defaulting to asymmetric `RS*`/`PS*`/`ES*`/`EdDSA`)
- `iss` mismatch
- `aud` mismatch (audience must match `config.audience`, or `config.appSlug` when `audience` is not set)
- `azp` (authorized party) mismatch — when present it must equal `config.appSlug`; in resource-audience mode (`config.audience` set and different from `appSlug`) it is **required** (`JWTClaimValidationFailed`)
