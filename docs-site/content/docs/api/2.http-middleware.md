...
| `request.org_id` | `string` | Active organization ID from the JWT `org_id` claim (only set when present) |
...
| `ERR_ACCOUNT_DISABLED` | 403 | User account `isDisabled === true` in the local DB |
