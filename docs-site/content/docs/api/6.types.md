...
interface OidcConfig {
  ...
  /** OAuth2 client_id / application slug. Used to validate the `azp` (authorized party) claim. */
  ...
  /**
   * Expected `aud` claim (RFC 8707 resource mode). When set to a resource URL
   * that differs from `appSlug`, the `azp` claim becomes mandatory.
   * Defaults to `appSlug` for backward compatibility.
   */
  readonly audience?: string;

  /**
   * Allow-list of JWS algorithms. Defaults to asymmetric only
   * (`RS*`, `PS*`, `ES*`, `EdDSA`); `HS*` and `none` are excluded.
   */
...
| `appSlug` | Yes | Must match the `azp` (authorized party) claim in the JWT |
| `jwksUri` | No | Full URL to the JWKS endpoint |
| `issuer` | No | Expected `iss` claim value |
| `audience` | No | Expected `aud` claim (RFC 8707 resource mode); defaults to `appSlug` |
| `algorithms` | No | JWS algorithm allow-list; defaults to asymmetric only |
...
interface OidcUserContext {
  ...
  /** Active organization ID from the JWT `org_id` claim. Present only when the `org` scope was requested. */
  org_id?: string;
}
```
