...
3. Calls `verifyOidcToken(token, config)` — verifies the signature (asymmetric algorithm allow-list), `iss`, `aud`, `azp`, and expiry via remote JWKS
...
6. If `appHandle.session.registerSocket(socket, namespace, email, name)` is available, registers the connection in the session tracker
...
| `socket.org_id` | `string` | Active organization ID from the JWT `org_id` claim (only set when present) |
...
| `ERR_USER_PROVISION_FAILED` | `users.findOrCreate` threw an error |
